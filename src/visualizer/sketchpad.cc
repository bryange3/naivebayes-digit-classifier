...
Sketchpad::Sketchpad(const vec2& top_left_corner, size_t num_pixels_per_side,
    ...
    : top_left_corner_(top_left_corner),
      ...
      pixels(int(num_pixels_per_side_), std::vector<int>(int(num_pixels_per_side_), 0)) {}
...
void Sketchpad::Clear() {
  ...
  pixels.clear();
  std::vector<std::vector<int>> pixels_new(int(num_pixels_per_side_), std::vector<int>(int(num_pixels_per_side_), 0));
  pixels = pixels_new;
}

const std::vector<std::vector<int>>& Sketchpad::get_pixels() const {
  return pixels;
}
